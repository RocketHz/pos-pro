...
<body class="flex">
                    ...
                    <div class="w-96 bg-slate-900 border-l border-slate-700 flex flex-col">
                        <div class="p-6 border-b border-slate-700">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <label class="text-sm text-slate-400">Mesa:</label>
                                <select id="pos-table-selector" class="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-white focus:outline-none focus:border-blue-500 flex-1">
            ...
            <section id="reports-section" class="section-content p-8 animate-fade-in hidden">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="glass-panel rounded-xl p-6 lg:col-span-2">
                        <h3 class="text-lg font-semibold mb-4">Métodos de Pago (Hoy)</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center" id="payment-methods-container">
                            <!-- Generated by JS -->
                        </div>
                    </div>
                </div>
    ...
    <!-- Table Action Modal -->
    <div id="table-action-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50 p-4">
        <div class="glass-panel max-w-sm w-full rounded-xl overflow-hidden shadow-2xl border border-slate-700">
            <div class="p-6 border-b border-slate-700 flex justify-between items-center">
                <h3 class="text-xl font-bold text-white" id="table-action-title">Mesa</h3>
                <button id="btn-close-table-action" class="text-slate-400 hover:text-white transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>

            <div class="p-6 space-y-4">
                <input type="hidden" id="table-action-id" value="">

                <div class="text-center">
                    <i class="fas fa-chair text-5xl text-blue-400 mb-3"></i>
                    <p id="table-action-status" class="text-lg font-bold"></p>
                    <p class="text-sm text-slate-400" id="table-action-capacity"></p>
                </div>

                <div class="space-y-2">
                    <button id="table-btn-occupy" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-lg font-bold transition-colors">
                        <i class="fas fa-chair mr-2"></i>Ocupar e Ir a Venta
                    </button>
                    <button id="table-btn-free" class="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-bold transition-colors hidden">
                        <i class="fas fa-check mr-2"></i>Liberar Mesa
                    </button>
                    <button id="table-btn-reserve" class="w-full bg-amber-600 hover:bg-amber-700 text-white py-3 rounded-lg font-bold transition-colors hidden">
                        <i class="fas fa-bookmark mr-2"></i>Reservar Mesa
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- Alert Modal -->
    <div id="alert-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50 p-4">
        <div class="glass-panel max-w-sm w-full rounded-xl overflow-hidden shadow-2xl border border-slate-700">
            <div class="p-6 text-center">
                ...
                <button id="btn-close-alert" class="w-full btn-primary py-3 rounded-lg text-white font-bold">
                    Aceptar
                </button>
            </div>
        </div>
    ...
    <!-- Confirm Modal -->
    <div id="confirm-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50 p-4">
        <div class="glass-panel max-w-sm w-full rounded-xl overflow-hidden shadow-2xl border border-slate-700">
            <div class="p-6 text-center">
                <div class="w-16 h-16 rounded-full bg-amber-500/20 flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-question-circle text-3xl text-amber-400"></i>
                </div>
                <h3 id="confirm-title" class="text-lg font-bold text-white mb-2"></h3>
                <p id="confirm-message" class="text-slate-300 mb-6"></p>
                <div class="flex gap-3">
                    <button id="btn-confirm-no" class="flex-1 bg-slate-700 hover:bg-slate-600 text-white py-3 rounded-lg font-bold transition-colors">
                        Cancelar
                    </button>
                    <button id="btn-confirm-yes" class="flex-1 btn-primary py-3 rounded-lg text-white font-bold">
                        Confirmar
                    </button>
                </div>
            </div>
    ...
    <!-- Checkout Modal -->
    <div id="checkout-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50 p-4">
        <div class="glass-panel max-w-md w-full rounded-xl overflow-hidden shadow-2xl border border-slate-700">
            <div class="p-6 border-b border-slate-700 flex justify-between items-center">
                <h3 class="text-xl font-bold text-white"><i class="fas fa-credit-card mr-2 text-blue-400"></i>Procesar Pago</h3>
                <button id="btn-close-checkout" class="text-slate-400 hover:text-white transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <select id="checkout-payment-method" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500">
                        <option value="cash">💵 Efectivo</option>
                        <option value="pos">💳 Punto de Venta</option>
                        <option value="mobile_transfer">📱 Pago Móvil / Transferencia</option>
                        <option value="other">🔹 Otro</option>
                    </select>
                </div>

                <!-- Reference (shown for mobile_transfer) -->
                <div id="checkout-reference-group" class="hidden">
                    <label class="block text-sm text-slate-400 mb-2">Referencia <span class="text-red-400">*</span></label>
                    <input type="text" id="checkout-reference" placeholder="Número de referencia"
                           class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 uppercase">
                </div>

                <!-- Details (shown for other) -->
                <div id="checkout-details-group" class="hidden">
                    <label class="block text-sm text-slate-400 mb-2">Especifique el método <span class="text-red-400">*</span></label>
                    <input type="text" id="checkout-details" placeholder="Ej: Zelle, Binance, Divisas..."
                ...
                <!-- Summary -->
                <div class="bg-slate-800/50 rounded-lg p-4 space-y-2 text-sm">
                    <div class="flex justify-between text-slate-400">
                        <span>Subtotal</span>
                    ...
                    <div class="flex justify-between text-slate-400">
                        <span>Descuento</span>
                        <span class="text-emerald-400">-$<span id="checkout-discount">0.00</span></span>
                    </div>
                    <div class="flex justify-between text-slate-400">
                        <span>Propina</span>
                    ...
                    <div class="flex justify-between text-lg font-bold text-white pt-2 border-t border-slate-700">
                        ...
                        <span class="text-emerald-400">$<span id="checkout-total">0.00</span></span>
                    </div>
                </div>
            </div>

            <div class="p-6 border-t border-slate-700 bg-slate-800/50">
                <button id="btn-submit-checkout" class="w-full btn-primary py-3 rounded-lg text-white font-bold text-lg">
    ...
    <!-- Product Modal -->
    <div id="product-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-50 p-4">
        <div class="glass-panel max-w-lg w-full rounded-xl overflow-hidden shadow-2xl border border-slate-700 max-h-[90vh] flex flex-col">
            <div class="p-6 border-b border-slate-700 flex justify-between items-center">
                <h3 class="text-xl font-bold text-white" id="product-modal-title">Nuevo Producto</h3>
                <button id="btn-close-product-modal" class="text-slate-400 hover:text-white transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <!-- Emoji Selector -->
                <div>
                    <label class="block text-sm text-slate-400 mb-2">Icono del Producto</label>
                    <div id="product-emoji-grid" class="grid grid-cols-8 gap-1 max-h-32 overflow-y-auto bg-slate-800/50 rounded-lg p-2">
                    </div>
                    <input type="hidden" id="product-emoji" value="🍽️">
                </div>

                <!-- Name -->
                <div>
                    <label class="block text-sm text-slate-400 mb-1">Nombre <span class="text-red-400">*</span></label>
                    <input type="text" id="product-name" placeholder="Ej: Hamburguesa Clásica"
                ...
                <!-- Category -->
                <div>
                    <label class="block text-sm text-slate-400 mb-1">Categoría <span class="text-red-400">*</span></label>
                    <select id="product-category" class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm text-slate-400 mb-1">Precio ($) <span class="text-red-400">*</span></label>
                        <input type="number" step="0.01" id="product-price" placeholder="0.00"
                               class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm text-slate-400 mb-1">Stock <span class="text-red-400">*</span></label>
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="product-is-active" checked class="sr-only peer">
                    ...
                    <span class="text-sm text-slate-300">Producto activo</span>
                </div>
            </div>

            <div class="p-6 border-t border-slate-700 bg-slate-800/50">
                <button id="btn-submit-product" class="w-full btn-primary py-3 rounded-lg text-white font-bold">
            ...
            <div class="p-8" style="background-image: radial-gradient(#e5e7eb 1px, transparent 1px); background-size: 10px 10px;">
                ...
                <div class="space-y-1 text-sm">
                    <div class="flex justify-between">
                        <span>Subtotal:</span>
                        <span>$<span id="r-subtotal">0</span></span>
                    </div>
                    <div class="flex justify-between" id="receipt-discount-row">
                        <span>Descuento:</span>
                        <span class="text-green-600">-$<span id="r-discount">0</span></span>
                    </div>
                    <div class="flex justify-between">
                        <span>Propina:</span>
                        <span>$<span id="r-tip">0</span></span>
                    </div>
                ...
                <div class="mt-8 text-center">
                    <p class="text-xs text-gray-500">Método: <span id="receipt-payment-method">Efectivo</span></p>
